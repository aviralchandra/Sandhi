...
{
    const std::string root = my_get_env("GRAS_ROOT", "/usr/local");
...
GRAS_STATIC_BLOCK(gras_module_loader)
{
    ...
    load_modules_from_paths(my_get_env("GRAS_ROOT", "/usr/local"), fs::path("") / "lib" / "gras" / "modules");

    //!search the GRAS_PATH search directories for modules
    load_modules_from_paths(my_get_env("GRAS_PATH", ""), fs::path("") / "lib" / "gras" / "modules");
